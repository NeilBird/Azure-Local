...
|---|---|---|
| **Placement inconsistency** | Either complete inventory found no authoritative VM or snapshot reference for a disk located inside a VM-associated folder, or none of the VMs associated with the containing folder is an authoritative owner of the referenced disk. The report names the authoritative owner(s), associated-folder VM(s), and exact trigger. Filename tokens are never treated as ownership evidence. An attached disk outside all detected VM folders is allowed because dedicated/shared disk directories and separate CSV layouts can be intentional. A shared directory associated with multiple VMs, including the disk's authoritative owner, is also allowed; this covers layouts such as an AKS Arc RP workload directory shared by its control-plane and worker VMs. | For an unreferenced disk, determine whether it is a required image, retired workload disk, backup/live-mount artifact, or intentionally retained data. For an owner/folder mismatch, confirm whether the authoritative VM reference and different folder association are deliberate. Do not attach, move, rename, or delete the file based only on this report. |
