...
- The repo-only `Apply-ModuleDevelopmentChannel.ps1` maintainer helper validates an exact PSGallery candidate and uses GitHub CLI to create, update, or remove the `REQUIRED_MODULE_VERSION` repository variable. The dropped and maintainer `Update-Module-And-Pipelines.ps1` scripts now accept `-RequiredVersion` to install, import, and refresh pipelines from an exact version, including an unlisted candidate.
