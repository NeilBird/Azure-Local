...
- **`Export-AzLocalUpdateRunMonitorReport` now classifies a stalled run as a dedicated
  ...
  `LongRunningStep` / `LongRunningOverall` reasons. The CSV, markdown step summary, and
  `-PassThru` shape are unchanged. `GENERATED_AGAINST_MODULE_VERSION` bumped to `'0.8.96'`.
