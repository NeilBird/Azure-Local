function Export-AzLocalFleetUpdateStatusReport {
    ...
    $sbeDistinctVersions = @($sbeVersionDistribution | Select-Object -ExpandProperty Version -Unique).Count
    ...
    if ($IncludeUpdateRuns) {
        ...
        $allRuns = Invoke-AzLocalPipelineTimedOperation `
            ...
            -Enabled $timingEnabled `
            -ScriptBlock { Get-AzLocalUpdateRuns -ClusterResourceIds $fleetResourceIds -ExportPath $runsCsv -PassThru -SkipSideloadedReset -SuppressFormattedOutput 6>$null }
        $allRunsList = @($allRuns)
    ...
    $healthFailureCount = @($readiness | Where-Object { $_.HealthState -eq 'Failure' }).Count
    if ($healthFailureCount -gt 0) { $actions += "- **$healthFailureCount cluster(s) have HealthState=Failure** - Resolve critical health check issues in Azure Portal before retrying any updates. This includes clusters also counted under Update Failed." }
    ...
    [void]$md.Add('')
    [void]$md.Add('_JUnit time values for update history represent historical update-run durations, not the execution time of this reporting pipeline._')
    [void]$md.Add("_Generated at ${generatedUtc}_")
