function New-AzLocalApplyUpdatesScheduleConfig {
    <#
    .SYNOPSIS
        Generates a STRAWMAN apply-updates-schedule.yml (schema v3) from
...
"@
    ...
    [void]$sb.AppendLine('# =====================================================================')
    [void]$sb.AppendLine('# apply-updates-schedule.yml - schema v3   *** STRAWMAN - REVIEW REQUIRED ***')
    ...
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine('#   * apply-updates.yml           - reads it at every cron firing and')
    [void]$sb.AppendLine('#                                   resolves the UpdateRingValue to use')
    [void]$sb.AppendLine('#                                   for that run.')
    [void]$sb.AppendLine('#   * apply-updates-schedule-audit.yml')
    [void]$sb.AppendLine('#                                 - audits this file against the live')
    [void]$sb.AppendLine('#                                   fleet (UpdateRing + UpdateStartWindow')
    ...
    [void]$sb.AppendLine('#   2. The apply-updates cron schedule (intra-day-grain): says HOW OFTEN the')
    [void]$sb.AppendLine('#      apply-updates job wakes up (e.g. hourly).')
    [void]$sb.AppendLine('#   3. Per-cluster `UpdateStartWindow` tag (minute-grain): says WHEN, during')
    ...
    [void]$sb.AppendLine('# =====================================================================')
    [void]$sb.AppendLine('')
    [void]$sb.AppendLine('schemaVersion:        3')
    [void]$sb.AppendLine('')
    [void]$sb.AppendLine('# ---- AllowedUpdateVersions (schema v2, MANDATORY top-level) -------')
    [void]$sb.AppendLine('# >>> ALLOWED-UPDATE-VERSIONS-V2 <<<')
    [void]$sb.AppendLine('# Fleet-wide allow-list of Azure Local solution-update names or')
    [void]$sb.AppendLine('# version strings that apply-updates is permitted to install.')
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine("# Default 'Latest' (case-insensitive) is a reserved sentinel meaning")
    [void]$sb.AppendLine("# 'no constraint - install the latest Ready update on each cluster'.")
    [void]$sb.AppendLine('# Keeping it as Latest preserves the pre-v2 behaviour.')
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine("# To enforce a 'minimum updates' policy (~4 updates per year - YY04 +")
    [void]$sb.AppendLine('# YY10 feature updates plus the preceding YY03 + YY09 cumulative')
    [void]$sb.AppendLine("# updates), replace 'Latest' with a semicolon-separated list of")
    [void]$sb.AppendLine('# explicit update names or version strings. Clusters with no Ready')
    [void]$sb.AppendLine("# update matching the list are SKIPPED with status 'NotInAllowList'")
    [void]$sb.AppendLine("# (strict no-op; never falls back to 'latest').")
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine("# Format: semicolon-separated string (same convention as 'rings:').")
    [void]$sb.AppendLine('# Per-row override: any schedule row below may set its own')
    [void]$sb.AppendLine("# 'allowedUpdateVersions:' field. Per-row beats top-level; multiple")
    [void]$sb.AppendLine("# matching rows UNION their lists; rows without the field on a UNION")
    [void]$sb.AppendLine("# day are treated as 'no opinion' (not 'allow nothing'); if any")
    [void]$sb.AppendLine("# matching row contributes 'Latest', the effective list is 'Latest'")
    [void]$sb.AppendLine('# (no constraint).')
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine('# Example (replace the line below with explicit versions to opt-in):')
    [void]$sb.AppendLine("#   allowedUpdateVersions: '10.2604.0.123;10.2610.0.456'")
    [void]$sb.AppendLine("allowedUpdateVersions: 'Latest'")
    [void]$sb.AppendLine('')
    [void]$sb.AppendLine('# ---- Prepare before install (schema v3, MANDATORY) ------------')
    [void]$sb.AppendLine('# >>> PREPARE-ONLY-FIRST-V3 <<<')
    [void]$sb.AppendLine('# false preserves the existing one-step apply behavior.')
    [void]$sb.AppendLine('# true uses the Azure Local 2608 prepare updates workflow: Ready')
    [void]$sb.AppendLine('# updates are prepared first without enforcing UpdateStartWindow, while')
    [void]$sb.AppendLine('# UpdateExclusionsWindow remains a hard blackout. A later firing installs')
    [void]$sb.AppendLine('# from ReadyToInstall only when both schedule gates permit installation.')
    [void]$sb.AppendLine('# A schedule row may override this boolean for that firing.')
    [void]$sb.AppendLine('prepareOnlyFirst: false')
    [void]$sb.AppendLine('# true allows out-of-window preparation; false requires the')
    [void]$sb.AppendLine('# preparation phase to pass the UpdateStartWindow gate.')
    [void]$sb.AppendLine('allowPrepareOnlyOutsideOfUpdateStartWindow: true')
    [void]$sb.AppendLine('')
    ...
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine('# Detailed guide:')
    [void]$sb.AppendLine('# https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/Automation-Pipeline-Examples/docs/apply-updates-schedule.md')
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine('# ISO weeks run Monday through Sunday. ISO week 1 contains the year''s')
    [void]$sb.AppendLine('# first Thursday, so dates near a year boundary can belong to the')
    [void]$sb.AppendLine('# adjacent ISO year. Always use both the ISO week and ISO year.')
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine("# For reference, ISO week 1 of $($iso.Year) began on $w1MonStr.")
    ...
    [void]$sb.AppendLine('# Keep the anchor fixed after committing the file; do not advance it')
    [void]$sb.AppendLine('# each week. Four weeks is a 28-day repeating operating pattern, not')
    [void]$sb.AppendLine('# a calendar month; use a longer cycle for additional soak time.')
    ...
    [void]$sb.AppendLine('# Common setup: put all rings sharing the same week and days in one')
    [void]$sb.AppendLine('# semicolon-separated row, e.g. rings: ''Canary;DevTest''. This selects')
    [void]$sb.AppendLine('# every cluster carrying either UpdateRing tag value.')
    [void]$sb.AppendLine('# Optional: use multiple rows with the same weeksInCycle value when')
    [void]$sb.AppendLine('# rings need different days or row-level policies. Matches on the same')
    [void]$sb.AppendLine('# week and day are unioned and deduplicated.')
    [void]$sb.AppendLine('#')
    ...
    [void]$sb.AppendLine("#   allowedUpdateVersions  OPTIONAL (schema v2). ';'-separated allow-list of update")
    [void]$sb.AppendLine("#                 names or version strings. When set on a row, it OVERRIDES the")
    [void]$sb.AppendLine("#                 top-level allowedUpdateVersions for that (cycleWeek, dayOfWeek)")
    [void]$sb.AppendLine('#                 match. Useful for per-ring policies (e.g. canary ring gets new')
    [void]$sb.AppendLine('#                 cumulative immediately, prod ring waits for the feature update).')
    [void]$sb.AppendLine('#   prepareOnlyFirst OPTIONAL (schema v3). true or false. Overrides the')
    [void]$sb.AppendLine('#                 top-level policy for this row. Matching rows must agree.')
    [void]$sb.AppendLine('#   allowPrepareOnlyOutsideOfUpdateStartWindow OPTIONAL (schema v3).')
    [void]$sb.AppendLine('#                 true bypasses UpdateStartWindow during preparation; false')
    [void]$sb.AppendLine('#                 enforces it. Matching row overrides must agree.')
    ...
    for ($i = 0; $i -lt $sorted.Count; $i++) {
        ...
        [void]$sb.AppendLine("  #   # allowedUpdateVersions: '10.2604.0.123;10.2610.0.456'  # optional row-level override (schema v2)")
        [void]$sb.AppendLine('  #   # prepareOnlyFirst: true  # optional row-level override (schema v3)')
        [void]$sb.AppendLine('  #   # allowPrepareOnlyOutsideOfUpdateStartWindow: false  # optional row override (schema v3)')
    }
    [void]$sb.AppendLine('  # ---------------------------------------------------------------')
