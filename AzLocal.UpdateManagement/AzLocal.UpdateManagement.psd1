@{
    ...
    PrivateData = @{
        PSData = @{
            ...
            ReleaseNotes = @'
## Version 0.7.92 - Step.9 fleet-health step summary: per-cluster collapsible details + Step.7 monitor-updates default schedule (5x/day, every 2h overnight)

Docs/YAML-only feature release. No cmdlet behaviour changes, no schema changes, no breaking changes. The `Step.9_fleet-health-status.yml` and `Step.7_monitor-updates.yml` pipelines (GitHub Actions + Azure DevOps) are the only things that move.
...
Apply via `Update-AzLocalPipelineExample -Destination <path>` to refresh the `Step.9_*.yml` and `Step.7_*.yml` files. The Step.9 changes sit outside any `BEGIN-AZLOCAL-CUSTOMIZE` block, so operator customisations elsewhere in those pipelines are preserved.
