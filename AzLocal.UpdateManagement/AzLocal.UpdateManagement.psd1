@{
    ...
    PrivateData = @{
        PSData = @{
            ...
            ReleaseNotes = @'
## Version 0.7.92 - Step.9 fleet-health step summary: per-cluster collapsible details + Step.7 monitor-updates default schedule (5x/day, every 2h overnight) + pipeline step-summary hyperlinks open in a new tab
...
- **Step.7 ships with an active default schedule.** `Step.7_monitor-updates.yml` (GH + ADO) now runs 5x/day at 20:00, 22:00, 00:00, 02:00, 04:00 UTC - every 2 hours across the typical overnight maintenance window - in addition to manual `workflow_dispatch` / queue runs. Previously the schedule block shipped commented-out and required operators to enable it per estate. The cron lives inside the existing `BEGIN-AZLOCAL-CUSTOMIZE:schedule-triggers` block, so any local edits (hour-list shifts for a non-UTC maintenance window, weekends-only, sub-hour external triggers, etc.) survive `Update-AzLocalPipelineExample` refreshes.
- **`Copy-AzLocalPipelineExample` post-copy hint now calls out `apply-updates-schedule.yml`.** New explicit Next-step line in both the `-Platform GitHub` and `-Platform AzureDevOps` summaries reminds operators that the ring-aware schedule file is required for **scheduled** Step.6 runs (manual `workflow_dispatch` runs still work without it) and points at `New-AzLocalApplyUpdatesScheduleConfig -OutputPath ...`. The bundled `apply-updates-schedule.example.yml` is deliberately not copied (it is documentation only - the real file must reflect each estate''s live `UpdateRing` tag values). The same step is now inline in `Automation-Pipeline-Examples/README.md` sections 5.1 (GitHub Actions) and 5.2 (Azure DevOps), so the information is no longer scattered between section 8 and the example file header.
...
- **Step.3 schedule-audit summary metric table now reconciles when `-IncludeUntagged` is set.** The summary metric table previously surfaced only 7 of the 8 possible `Status` buckets - the `NoWindowTag` row (clusters with no `UpdateStartWindow` tag, emitted when `IncludeUntagged: true` is set on the workflow input) was missing. Result: `(Ring, Window) pairs audited` did not equal the sum of the visible bucket counts (e.g. audited=7 but Covered+Uncovered+... summed to 6), which made the table look broken even though the underlying data and the audit detail table below were correct. v0.7.92 adds a `NoWindowTag` row to the metric table (both GH + ADO Step.3) and includes it in the `hasIssues` calculation so the action-required Recommend block surfaces when untagged clusters are present.
