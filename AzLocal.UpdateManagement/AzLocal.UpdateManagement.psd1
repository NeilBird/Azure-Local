@{
    ...
    NestedModules = @(
        ...
        'Private/Repair-AzLocalAzureCliAuthentication.ps1',
...
## Version 0.9.38 - Reporting accuracy: schedule recommendations distinguish optional windows from ring-scoped opt-in; readiness labels reflect actual scope and cached evidence. Connectivity summaries expose observed NIC coverage and qualify unmatched ARBs. Health counts describe occurrences and JUnit messages avoid duplicate severity. Failed-run links include encoded update/run selectors; monitor JUnit distinguishes recent from older unresolved failures. Fleet summaries count distinct SBE values and overlapping health failures correctly and restore the generated timestamp. Reporting pipeline exit policy, raw records, and 73 exports are unchanged. Bundled pipeline pins: 0.9.38.
            ...
            GitHub JUnit report details now start collapsed under an explanatory heading; counts, artifacts, ITSM, and check outcomes are preserved. Azure DevOps retains its native Tests view.

## Version 0.9.37 - Sideload profiles add optional ioRateBytesPerSecond (/IORATE, runner support checked) and detailedLogging controls. Diagnostics-enabled GitHub Actions and Azure DevOps sideload pipelines attach bounded robocopy log ZIPs with allowlisted state metadata. Detached copies fix spaced-path and trailing-backslash quoting and attempt child-process cleanup on worker failure. Setup, recovery, and remote pilot guidance is expanded. Apply/prepare require successful CLI exit codes; fleet jobs fix private-helper scope and pass-through results; update-run history follows validated continuation links. ITSM hardens HTTPS validation, redirects, read retries, and incident deduplication; Azure DevOps passes free-form inputs through environment variables. Release gates reject discovery/setup failures, empty runs, and incomplete live shards; Windows PowerShell 5.1 regressions are covered. Fixes Config: 2 dry-run cleanup and summary reconciliation, and adds Get-AzLocalUpdateRuns -SuppressFormattedOutput for Monitor: 3 without changing exported data. No export-count change (73).
...
## Version 0.9.21 - Health summaries count clusters once by highest severity, split Critical and Warning-only counts, and clarify Other states. Health and connectivity tables use bare status icons. Adds critical_clusters and warning_only_clusters step outputs and matching PassThru properties. No export change (69).
...
## Version 0.9.18 - Guards optional fields in recursive update-step walkers and fleet readers under strict mode. Adds Add-AzLocalPipelineSupportFooter across all 20 templates and an install-banner support caveat. Exports 68 -> 69.
