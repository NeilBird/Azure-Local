@{
    ...
    PrivateData = @{
        PSData = @{
            ...
            ReleaseNotes = @'
## Version 0.8.75 - Patch: removes the duplicate Cycle calendar from the Step.3 apply-updates schedule audit step summary (the Recommend view's plain 5-column calendar was being inlined ABOVE the wrapper's own enriched 7-column calendar). New additive `-OmitCycleCalendar` switch on `Test-AzLocalApplyUpdatesScheduleCoverage`; `Export-AzLocalApplyUpdatesScheduleAudit` opts in so only the enriched calendar surfaces. Direct callers of the Recommend view see no behaviour change. Drift-notice banner now recommends `Update-AzLocalPipelineExample` (marker-aware merge) with the right `-Platform` and a `-Destination` hint inferred from the detected pipeline host. Section 8.3 of `Automation-Pipeline-Examples/README.md` gains three Step.3 step-summary screenshots. No public API or export-count change (still 60).

- **FIX (Step.3 step summary)**: the cycle-calendar table was rendered twice - once by `Test-AzLocalApplyUpdatesScheduleCoverage -View Recommend` (plain 5-col, inlined into the wrapper as the action-required block), and once by the wrapper itself (enriched 7-col with per-day cron firings + `Tag Start Window Match (>=95%)`). Additive `[switch]$OmitCycleCalendar` parameter on `Test-AzLocalApplyUpdatesScheduleCoverage` (default off) suppresses the inner emission; `Export-AzLocalApplyUpdatesScheduleAudit` opts in. New Pester regression asserts the switch is bound + exactly one `## Cycle calendar - next` heading rendered.
- **CHANGED (drift-notice banner)**: `Add-AzLocalPipelineVersionBanner` now recommends `Update-AzLocalPipelineExample` (marker-aware merge, preserves `AZLOCAL-CUSTOMIZE` blocks) instead of `Copy-AzLocalPipelineExample -Update` (clean overwrite). Inlines the right `-Platform` and `-Destination` hint per detected host: `'<repo-root>\.github\workflows' -Platform GitHub` on GH, `'<repo-root>\pipelines' -Platform AzureDevOps` on ADO. Symmetric YAML-newer warning gets the same treatment. PassThru verdict is `'YAML older than module - run Update-AzLocalPipelineExample to refresh'`.
- **DOCS**: 3 Step.3 screenshots added to `Automation-Pipeline-Examples/README.md` section 8.3 (cron coverage, NoWindowTag, enriched cycle calendar). `docs/images/README.md` referenced-by table updated.
