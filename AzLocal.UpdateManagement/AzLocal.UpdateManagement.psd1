...
- **NEW Pester regression guards** (would have caught all bugs pre-release):
  - `v0.8.6 regression guard: @() wrap on unary-comma return helpers` - 4 It blocks scanning every Public/Private `.ps1` for `@(\s*<Helper>\b` (helpers: `Invoke-AzResourceGraphQuery`, `Get-AzLocalFleetHealthFailures`, `Get-AzLocalFleetHealthOverview`, `Read-AzLocalApplyUpdatesYamlCrons`).
  - `v0.8.6 regression guard: Step.3 Test-AzLocalApplyUpdatesScheduleCoverage MatchingCrons strict-mode crash` - asserts explicit `ForEach-Object { $_.MatchingCrons }` enumeration is present and array-level member-access is gone.
  - `v0.8.6 regression guard: Step.4 ARB strict-mode .Count crash on single-cluster RG` - asserts the explicit `if/else { $matched = @(...) } else { $matched = @() }` branch is present (because `@($null)` yields `Object[1]` containing `$null`). Blocks both the v0.8.6-fix1 `[object[]]` cast and the v0.8.6-fix2 `$matchedList` patterns.
  - `v0.8.6 regression guard: Step.6 Get-AzLocalClusterUpdateReadiness $cluster.NotFound strict-mode crash` - parses every `$clustersToProcess += @{ }` literal in source and asserts each defines a `NotFound` key.
  - `v0.8.6 regression guard: Step.0 Export-AzLocalAuthValidationReport defensive @() coerce on $clusterRows` - asserts the variable-level coerce is present.
  - `v0.8.6 regression guard: GitHub Actions Node.js 20 deprecation` - 2 It blocks scanning every bundled GHA YAML for `actions/upload-artifact@v[1-4]` and `actions/download-artifact@v[1-3]`.
  - `v0.8.6 functional regression: Export-AzLocalAuthValidationReport` + `Export-AzLocalFleetHealthStatusReport` source-shape asserts.
