...
## Version 0.9.0 - Managed repo README auto-drop. `Copy-AzLocalPipelineExample` and `Update-AzLocalPipelineExample` now also drop a lightweight, link-first `README.md` into the customer REPO ROOT so a freshly set-up pipelines repo explains itself (what it is, how to refresh after a module release, where the docs live). Operator content is never destroyed: the README is written only when the repo has NO usable README, and a README carrying the hidden `<!-- AZLOCAL-README-VERSION: x.y.z -->` marker is version-gate refreshed in place only when the bundled template is newer. Default-on for single-platform layouts, suppressed by `-SkipReadme`. Additive - no export-count change (still 64). `GENERATED_AGAINST_MODULE_VERSION` bumped to `'0.9.0'`.
