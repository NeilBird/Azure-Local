...
- **`Copy-AzLocalPipelineExample` post-copy hint now calls out `apply-updates-schedule.yml`.** New explicit Next-step line in both `-Platform GitHub` and `-Platform AzureDevOps` summaries reminds operators that the ring-aware schedule file is required for **scheduled** Step.6 runs (manual `workflow_dispatch` runs still work without it) and points at `New-AzLocalApplyUpdatesScheduleConfig -OutputPath ...`. The bundled `apply-updates-schedule.example.yml` is deliberately not copied (the real file must reflect each estate''s live `UpdateRing` tag values). Same step is now inline in `Automation-Pipeline-Examples/README.md` sections 5.1 and 5.2.
...
- **Step.3 schedule-audit: belt-and-braces retry crons + `RingMixedWindows` warning.** New `-RecommendFiresPerWindow` parameter on `Test-AzLocalApplyUpdatesScheduleCoverage` (default `2`) and matching `fires_per_window` (GH) / `firesPerWindow` (ADO) workflow input on `Step.3_apply-updates-schedule-audit.yml`. When `2` (default), the Recommend snippet emits each window as TWO crons: `(open)` `LeadTimeMinutes` before the window opens, plus `(retry)` INSIDE the window at the lesser of midpoint or +60 minutes after open. Catches GH Actions scheduled-workflow jitter (~15 min), transient first-fire failures, and the long-window case where a midpoint retry would otherwise wait half the window. Runtime gate + in-flight guard prevent double-triggering. Pass `1` for pre-v0.7.92 single-cron behaviour. Audit semantics ("Covered" / "Uncovered") are unchanged in either mode. Also new: `RingMixedWindows` informational Audit status emitted when 2+ clusters share an `UpdateRing` tag but carry DIFFERENT `UpdateStartWindow` values (follow-the-sun, regional rollouts, tagging mistakes). Each (Ring, Window) pair still gets its own per-pair coverage row above; not counted as `Uncovered`.
