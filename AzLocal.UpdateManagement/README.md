...
**New Public cmdlet `Get-AzLocalApplyUpdatesScheduleCycleCalendar` + Step.6 manual schedule-file inputs + Step.3 cycle-calendar regression fix + per-ring cluster-count column.** One new exported cmdlet, two new Step.6 manual-run inputs; no other public API removed; no parameter changes on existing cmdlets.
