...
- **Safe automatic schema migration.** Schema v1 remains fully supported. A normal `Update-AzLocalPipelineExample` run saves the exact original file as `config/fleet-settings_v1.bak.yml`, changes the active version declaration to v2, and appends the new `clusterTagFilters` properties as a fully commented example. Existing operator values, comments, order, and line endings are preserved; reruns are idempotent and `-WhatIf` is supported. The former `-UpgradeFleetSettingsSchema` switch remains accepted for compatibility.
