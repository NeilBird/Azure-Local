...
**Version banner appended to every install step (GitHub Actions + Azure DevOps).** `_Pipeline YAML v<generated> | Module v<installed> installed (<pin>) | PSGallery latest <latestStr> | <verdict>_` is now appended to the Step Summary (GH) / Build Summary (ADO) by every install step on both platforms - 10 GH yml + 11 ADO install sites (Step.6 has 2: check-readiness install and apply-updates install). Surfaces "what version actually ran" in the Summary view fleet operators open first, eliminating "the YAML in my repo is too old" investigation lag.
