...
- [Release process](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/docs/RELEASE-PROCESS.md) - how to cut a release (maintainer-facing; not packaged)
...
> **For CI/CD?** Go straight to [Automation-Pipeline-Examples/README.md](./Automation-Pipeline-Examples/README.md) for OIDC / Managed Identity / Service Principal setup, federated credentials, and the GitHub Actions and Azure DevOps pipeline templates (including `sideload-updates`, `apply-updates`, `monitor-updates`, `fleet-update-status`, and `fleet-health-status`). Display names group workflows under `Config`, `Monitor`, and `Update`. Pipeline filenames no longer use `Step.N_`; `Update-AzLocalPipelineExample` migrates older filenames while preserving supported customizations.
...
- **Azure CLI** (`az`) 2.78.0 or later installed and authenticated (2.90.0 or later recommended)
...
**Authentication and input failures now fail clearly.** Concurrent GitHub Actions workers serialize OIDC repair and recheck the shared Azure CLI token cache before logging in again. Azure CLI preflight requires 2.78.0 or later and recommends 2.90.0 or later. Failed single-cluster available-update reads retain their ARM error instead of becoming an empty result, and manual Apply runs require either a full update resource name or `latest`. No public function or export-count change (73); all bundled pipeline pins are `0.9.35`. See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0935---2026-09-17) for full details.

### What's New in v0.9.34

**Config: 1 inventory and Config: 2 tag management now renew expired workload-identity authentication during long fleet runs.** Their long-running GitHub Actions steps expose the client, tenant, and default-subscription metadata required by the shared ARM transport to request a fresh runner OIDC assertion and retry the interrupted request once. Their matching Azure DevOps `AzureCLI@2` tasks enable session keepalive, refreshing Workload Identity Federation login every eight minutes.

**The fix preserves existing fleet and security boundaries.** Inventory remains read-only, tag reconciliation remains idempotent, the GitHub default-subscription value restores CLI context without narrowing fleet scope, and no credentials are stored in the workflow or module. The Azure DevOps keepalive requires a Workload Identity Federation ARM service connection. No public function or export-count change (73); all bundled pipeline pins are `0.9.34`. See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0934---2026-09-07) for full details.

### What's New in v0.9.33

**Azure Local 2608 release exposes update preparation through a control-plane API POST action.** Preparation was already available locally through PowerShell; v0.9.33 adds remote module and pipeline orchestration through the stable `POST .../prepare?api-version=2026-04-30` action. `Start-AzLocalClusterUpdate -PrepareOnly` downloads, validates/extracts, and health-checks a Ready update without installing it. `-PrepareOnlyFirst` provides the automated two-firing policy: a Ready update is prepared, then a later run applies it after Azure reports `ReadyToInstall`. `-AllowPrepareOnlyOutsideOfUpdateStartWindow $true` allows preparation before or outside the installation window; `$false` enforces the start window. Preparation always honors `UpdateExclusionsWindow` and the independent `UpdateExcluded` operator hold.

**Apply schedule schema v3 makes both preparation policies source-controlled.** Mandatory top-level `prepareOnlyFirst: false` selects direct apply, while `allowPrepareOnlyOutsideOfUpdateStartWindow: true` allows preparation ahead of the installation window. Optional row values override either policy for that firing, and conflicting matching rows fail closed. Normal `Update-Module-And-Pipelines.ps1` refreshes migrate v1/v2 schedules through every required hop, save the exact original as `config/apply-updates-schedule.v<old>.old.yml`, preserve the operator-owned `schedule:` section and newline style byte-for-byte, validate the new canonical file, and restore the original automatically on failure.

**Config: 3 identifies preparation opportunities.** When a schedule row effectively resolves both schema-v3 policies to true, the coverage audit recommends a deduplicated Apply Updates cron six hours before the corresponding `UpdateStartWindow`. It emits no extra cron when preparation must remain inside the start window and flags previous-day rollovers that require the ring to be eligible on the preparation day.
...
**Pipeline transcripts no longer report false parser or role-query failures.** Config: 1 handles a successful empty role-assignment result as an empty result, Config: 3 excludes synthetic mixed-window display values from canonical tag parsing, and the Azure DevOps Apply Updates task binds its manual operation controls where they are consumed.
...
**Fixes a Bad Request in the "Update: 1 - Assess Update Readiness" pipeline.** `Sync-AzLocalClusterUpdateSummary` (and the `Export-AzLocalClusterUpdateReadinessReport` stale-assessment auto-scan that calls it) POSTed the `checkUpdates` ARM action with no request body; the `2026-03-01-preview` API spec now requires one. The cmdlet now sends an empty JSON object `{}` (no properties are mandatory for a plain re-scan), validated end-to-end against a live cluster, and refreshes the now-stale RBAC comment to reflect that the v0.8.92 `updateSummaries/*` wildcard already authorizes `checkUpdates`. Bug-fix only - no API, parameter, or export-count change (still 61). `GENERATED_AGAINST_MODULE_VERSION` bumped from `0.8.92` to `0.8.93`. See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0893---2026-06-18) for the full v0.8.93 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0873---2026-06-11) for the full v0.8.73 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0872---2026-06-11) for the full v0.8.72 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0871---2026-06-11) for the full v0.8.71 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#087---2026-06-11) for the full v0.8.7 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#086---2026-06-10) for the full v0.8.6 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0797---2026-06-08) for the full v0.7.97 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0796---2026-06-09) for the full v0.7.96 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0795---2026-06-08) for the full v0.7.95 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0794---2026-06-08) for the full v0.7.94 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0793---2026-06-05) for the full v0.7.93 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0792---2026-06-05) for the full v0.7.92 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0791---2026-06-05) for the full v0.7.91 entry.
...
**Six new per-pipeline smoke-test harnesses** in `Tools/` (Step.1/3/5/7/8/9) plus [Tools/SMOKE-COVERAGE.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/Tools/SMOKE-COVERAGE.md).

See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0790---2026-06-04) for the full v0.7.90 entry including migration recipe.
