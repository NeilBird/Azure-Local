    ...
    version is published to PowerShell Gallery to:
      ...
      2. Refresh the bundled pipeline YAMLs via Update-AzLocalPipelineExample -
         ...
         service-connection names, runner labels, ITSM secret bindings, etc.).
            If config\apply-updates-schedule.yml uses an older supported schema,
            the refresh saves the exact original as
            apply-updates-schedule.v<old>.old.yml, performs the additive migration,
            preserves schedule rows and line endings, validates the result, and
            restores the original automatically on failure.
...
.NOTES
    ...
    Template version: 1.4.1 (managed file - Copy/Update-AzLocalPipelineExample
