...
.DESCRIPTION
    ...
    version is published to PowerShell Gallery to:
      ...
      3. Stage ONLY the workflow folder and the repo-root config\ folder, then
...
.NOTES
    ...
    Template version: 1.0.0 (managed file - Copy/Update-AzLocalPipelineExample
...
# ---------------------------------------------------------------------------
# 3. Stage ONLY the workflow folder + repo-root config\, then commit and push
#    when there is something to commit. Scoping the 'git add' keeps unrelated
#    working-tree changes out of this automated commit. '-A -- <path>' also
#    captures the canonical-name renames/deletes that Update may perform.
# ---------------------------------------------------------------------------
if ($NoPush) {
