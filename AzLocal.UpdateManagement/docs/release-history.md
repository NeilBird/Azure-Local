...
**Sideload copy profiles add optional requested I/O rate and detailed logging.**
`ioRateBytesPerSecond` emits a bounded `/IORATE` only when the runner supports it;
`detailedLogging` adds file detail and retains the robocopy job header/summary.
Existing profiles keep their defaults, and raw arguments remain blocked. The
[copy guide](../Automation-Pipeline-Examples/docs/sideload-robocopy.md) explains safe
profiles and how pipeline artifacts differ from detached worker and agent-service logs.
When diagnostics are enabled, both templates now attach `sideload-copy-diagnostics.zip`
with bounded current-log snapshots and a collection manifest. This does not collect
runner-service logs or capture output written after the snapshot.

**Failure handling and evidence are hardened.** Apply/prepare success requires a zero
CLI exit code; fleet jobs use module-scoped helpers and structured apply results;
update-run history follows bounded same-resource pagination. ITSM requires HTTPS
without redirects, blocks creation when dedupe reads fail, and reconciles uncertain
POST outcomes without automatic write retries. Azure DevOps free-form inputs stay
data, sideload native paths are quoted, and test gates reject infrastructure failures
and incomplete shards. Setup guidance now includes explicit acceptance checkpoints
and a remote sideload pilot checklist; sideload end-to-end validation remains outstanding.

**Config: 2 dry runs now clean up their parallel jobs and report complete category totals.** Internal `Remove-Job` cleanup bypasses inherited `WhatIf`, so diagnostics no longer contain cleanup WhatIf records or retain completed process jobs until runner exit. The inner reconciliation summary now counts `WhatIf (dry-run)` rows explicitly.

**Monitor: 3 suppresses nested formatted output at its source.** `Get-AzLocalUpdateRuns` adds `-SuppressFormattedOutput`, which hides direct host tables and detailed objects while retaining result objects, exports, logs, warnings, errors, and diagnostic streams. Monitor: 3 uses it for update-run collection, keeping its concise summary and `update-runs.csv`. No public function or export-count change (73); pipeline pins are updated to `0.9.37`. See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0937---2026-09-18) for full details.
...
**Config: 2 pipeline status now reflects per-cluster failures after evidence is preserved.** GitHub Actions and Azure DevOps publish summaries and artifacts before a final `failed_count` gate fails the run, and the GitHub summary links directly to the diagnostics ZIP when upload succeeds. No public function or export-count change (73); pipeline pins are updated to `0.9.36`. See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0936---2026-09-18) for full details.

**Monitor collection costs are now visible without changing concurrency.** Monitor 1-3 append internal query and collection-stage durations to `pipeline-timings.json`. The same artifact adds non-secret run context and structured per-operation ARG fingerprints, aggregate scope counts, row/page totals, page sizing, truncation, cooldown, and retry diagnostics without storing KQL or scope identifiers. Monitor 3 also suppresses the redundant fleet-wide `Get-AzLocalUpdateRuns` host dump while preserving its CSV artifact and pass-through rows. The pipeline reference removes stale Monitor 2/3 `throttle_limit` inputs because these collectors use internal ARG batching. Transcript regression coverage verifies credential-shaped ARM and ARG failures remain scrubbed.
...
Patch release. Step.05 + Step.10 step-summary UX polish from the v0.8.81 manual pipeline-run review. No public API or export-count change (still 60 exports).

**Step.05 Summary counts table** (`Export-AzLocalClusterUpdateReadinessReport`) no longer duplicates row labels. Each row reused the shared `Get-AzLocalStatusIconMap` cell (which already includes its own label) AND appended a duplicate trailing label, producing `Ready for Update Ready for update` / `Up to Date Up to date` / `Action Required Not ready for update` / `Health Failure Clusters with Critical health failures`. The icon-map cell is now emitted unmodified. The HealthFailure row keeps `(Clusters with Critical health failures)` in parentheses since it counts something different from the readiness cascade.

**Step.05 All clusters detail table** now sorts by Status priority first (`InProgress` -> `HealthFailure` -> `UpdateFailed` -> `ActionRequired` -> `SbeBlocked` -> `NeedsInvestigation` -> `ReadyForUpdate` -> `UpToDate`), then `UpdateRing` + `ClusterName` as before. In-flight + remediation rows surface at the top; Up-to-Date drops to the bottom so operators see actionable items first.

**Step.05 Not-Ready clusters (review first) table - Blocking reasons column** no longer shows `-` for rows blocked by `UpdateFailed` / `NeedsAttention` / `InProgress` / Warning-only `HealthFailure` / `SbeBlocked`. The upstream `Get-AzLocalClusterUpdateReadiness` only populates `BlockingReasons` for `CriticalHealthCheck` findings and abnormal cluster connectivity states (e.g. `NotConnectedRecently`); every other Not-Ready category was left empty and rendered as `-`. The renderer now derives an actionable token from the Status bucket when `BlockingReasons` is empty (`UpdateInProgress (run in-flight)`, `UpdateState=<UpdateState>`, `UpdateState=PreparationFailed`, `HealthState=Failure (no Critical findings; review Warning findings)`, `PrerequisiteRequired (SBE update first)`, `NeedsInvestigation (no Update or Health signal)`), with `; HealthState=Warning` appended when relevant. The upstream object shape is unchanged; the derivation is renderer-side only so JSON / CSV / `-PassThru` consumers continue to see the original empty `BlockingReasons` value.

**Step.10 Detailed Results Description column** inline-vs-collapse threshold bumped from 120 to 280 characters in `Export-AzLocalFleetHealthStatusReport`. Short single-sentence descriptions render inline and only long multi-line descriptions collapse behind `<details><summary>view</summary>...</details>`. The previous 120-char cutoff put roughly half the rows inline and half collapsed on the same table, which looked broken.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0878---2026-06-15) for the full v0.8.78 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0877---2026-06-14) for the full v0.8.77 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0876---2026-06-12) for the full v0.8.76 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0875---2026-06-12) for the full v0.8.75 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#082---2026-06-10) for the full v0.8.2 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#081---2026-06-09) for the full v0.8.1 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#080---2026-06-09) for the full v0.8.0 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0799---2026-06-09) for the full v0.7.99 entry.
...
See [CHANGELOG.md](https://github.com/NeilBird/Azure-Local/blob/main/AzLocal.UpdateManagement/CHANGELOG.md#0798---2026-06-09) for the full v0.7.98 entry.
...
![Step.5_apply-updates.yml run Summary tab: target UpdateRing Prod, dry-run, Readiness section showing Total Clusters 9 / Ready for Update 4, Results section showing Updates Started 0 / Skipped 5 / Failed 0 / Health Blocked 0 / Schedule Blocked 0 / Sideloaded Blocked 0](images/apply-updates-summary.png)
...
- **Module version pin bumped to 0.7.60 in all 10 sample workflow YAMLs.** `GENERATED_AGAINST_MODULE_VERSION` is the value the in-pipeline install step compares the actually-installed module version against, then warns if your local copy of the YAML is stale (see [Automation-Pipeline-Examples/README.md section 5.3](../Automation-Pipeline-Examples/README.md#53-optional-configuration-not-recommended-pin-the-module-version)). All five GitHub Actions YAMLs and all five Azure DevOps YAMLs have been bumped in lock-step.
