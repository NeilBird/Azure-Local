function ConvertFrom-AzLocalUpdateExclusion {
    <#
    .SYNOPSIS
        Parses an UpdateWindowExclusions tag value into structured date range objects.
    .DESCRIPTION
        Parses the exclusion date range syntax used in the UpdateWindowExclusions Azure resource
    ...
    .PARAMETER ExclusionString
        The UpdateWindowExclusions tag value to parse.
    ...
    if ([string]::IsNullOrWhiteSpace($ExclusionString)) {
        throw "UpdateWindowExclusions value cannot be empty."
    }

    if ($ExclusionString.Length -gt 256) {
        throw "UpdateWindowExclusions value exceeds Azure tag limit of 256 characters (length: $($ExclusionString.Length))."
    }
